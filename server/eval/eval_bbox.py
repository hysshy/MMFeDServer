...
def eval(config, checkpoint, adaptive_w_dict, gpu_id=0, mertrick='bbox', tasktype='detect'):
    ...
    outputs = single_gpu_test(model, data_loader, False, None, 0.3, adaptive_w_dict=adaptive_w_dict)
    ...
    if rank == 0:
        ...
        if mertrick:
            ...
            eval_kwargs.update(dict(metric=mertrick, **kwargs))
            metric = dataset.evaluate(outputs, **eval_kwargs)
            logger.info('准确率测试 ' + tasktype + ':')
            logger.info(metric)
